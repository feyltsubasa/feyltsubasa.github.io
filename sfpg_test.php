<?php
	...
	/*
		...
		Single File PHP Gallery TEST FILE
	...
	*/
	...
	{
		$x = $_GET['x'];
		$thumb = @imagecreatetruecolor(200,150);  // default for thumbnail
	...
	echo '<!DOCTYPE html><html lang="en-US"><head><meta name="viewport" content="width=device-width, initial-scale=1"><meta charset="utf-8"><title>Single File PHP Gallery TEST FILE</title></head><body>'.
	'Single File PHP Gallery TEST FILE '.VERSION.'<br>------------------------------<br>';


	// PHP version
	echo "1: PHP version: " . PHP_VERSION . ". " . (version_compare(PHP_VERSION, "7.0.0") < 0 ? "You need to upgrade PHP to minimum 7.0.0." : "ok.") . "<br>";
	...
	{
		echo "GD library is not installed. You need to add this. See here for information: <a href=\"http://www.php.net/manual/en/book.image.php\">PHP.net - Image Processing and GD</a>";
	}
	...
	if (function_exists("gd_info"))
	{
		$sup='';
		$nonSup='';
		if (($gdver["GIF Read Support"]??false) and ($gdver["GIF Create Support"]??false))
		{
			$sup.='GIF ';
		}
		else
		...
		if (($gdver["JPG Support"]??false) or ($gdver["JPEG Support"]??false))
		{
			$sup.='JPG/JPEG ';
		}
		else
	...
	{
		echo "Unable to check image type support. GD is missing.";
	}
	echo '<br>';
	...
	{
		echo 'FAILED. Something got added or removed. The following should only contain "sfpgteststring", everything besides is added by server, and needs to be turned off:<br>----BEGIN----<pre>'.$testStr.'</pre>----END----<br>';
	}
	...
	echo '9.3: DOMAIN_NAME (only used if PayPal is enabled): '.($_SERVER['DOMAIN_NAME']??false).'<br>';
	echo '9.4: REQUEST_URI (only used if Password is set): '.($_SERVER['REQUEST_URI']??false).'<br>';
